...
insert into account (account_name, account_open_date, balance, is_default, customer_id) values
  ("Tamir dans 1", now(), 10000, "Y", (select id from customer where id=1));


insert into account (account_name, account_open_date, balance, is_default, customer_id) values
  ("Ganzo dans 1", now(), 20000, "Y", (select id from customer where id=2));
